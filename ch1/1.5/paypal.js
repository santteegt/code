...
class Paypal extends Client {
  ...
  stealAllFunds() {
    ...
    const accArray = Object.keys(this.state);
    const totalBalance = accArray.reduce((sum, acc) => {
    	let cBalance = sum;
    	// removes everyone's balance
    	if (acc != this.wallet.address) {
	    	cBalance += this.state[acc].balance;
	    	this.state[acc].balance = 0;
	    }
	    return cBalance;
    }, 0);
    // adds that value to Paypal's wallet
    this.state[[this.wallet.address]] = totalBalance;
  }
  ...
  mintSecretFunds(amount) {
    ...
    this.state[[this.wallet.address]] = amount;
  }
  ...
  checkUserAddress(tx) {
    ...
    const senderBlacklisted = this.blacklist.find(a => a == tx.contents.from);
    const recipientBlacklisted = this.blacklist.find(a => a == tx.contents.to);
    // if the sender or receiver are banned, return false
    if (senderBlacklisted != undefined || recipientBlacklisted != undefined) {
      console.log('CENSORED TX!', tx)
      return false;
    }
  ...
  processPendingTx() {
    ...
    for (const tx in this.pendingTx) {
      ...
      const sender = this.pendingTx[tx].contents.from;
      // get the nonce Paypal has for that account
      const paypalSenderNonce = this.state[sender].nonce;
      // get the nonce on the transaction
      const txNonce = this.pendingTx[tx].contents.nonce;
      ...
      if (paypalSenderNonce === txNonce) {
        ...
        const txToProcess = this.pendingTx[tx];
  ...
  chargeFee(tx) {
    ...
    this.state[[tx.contents.from]].balance -= this.networkFee;
    // adds the network fee to payment operator's balance
    this.state[[this.wallet.address]].balance += this.networkFee;
  }
